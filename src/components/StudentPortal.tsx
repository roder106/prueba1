...
export default function StudentPortal({ onSelectStudent, activeStudent, onLogout }: StudentPortalProps) {
  ...
  const handleLoginByRNE = (e: React.FormEvent) => {
    ...
    if (!cleanRne) {
      setLoginError('Por favor ingrese un RNE válido.');
      return;
    }
    ...
    if (found) {
      ...
      setLoginError('No se encontró ningún estudiante local con ese RNE. Verifique las siglas o regístrese como nuevo estudiante.');
    }
  };
